 ...
 * Author: ray
 * 
 * Created on September 3, 2014, 11:34 PM
 */
...
//        fprintf(result, ":- not reached(%d).\n", i);
    ...
    // 再加入右边的所有 :- not reached(a)。
    for(int i = m_pealSize; i < 2*m_pealSize; i++)
        fprintf(result, ":- not reached(%d).\n", i);
...
//    printf("\n");
    ...
    for(vector<Loop>::iterator it = allLoops.begin(); it != allLoops.end(); it++) {     
//        if(!((it->loopNodes).size() <= U.size() && includes(U.begin(), U.end(), (it->loopNodes).begin(), (it->loopNodes).end()))) {
            set<int> ENodes;
            set_intersection(U.begin(), U.end(), (it->loopNodes).begin(), (it->loopNodes).end(), inserter(ENodes, ENodes.begin()));
            Loop E;
            E.loopNodes = ENodes;
            if(!ENodes.empty())
                Es.push_back(E);
...
vector<Loop> DependenceGraph::HL_UPX(set<int> X) {
    ...
    for(vector<Loop>::iterator it = Es.begin(); it != Es.end(); it++) {
        ...
        if(((it->loopNodes).size() < X.size()) && 
            ...
            R_EX((*it), X);
            set<int> diff;
            set_difference((it->ESRules).begin(), (it->ESRules).end(), inUP.begin(), inUP.end(), inserter(diff, diff.begin()));
...
//            }
            ...
            // 测试：输出 R-(E,X) 和 in_U(P)
//            printf("\nR-(E.X) : \n");
//            for(set<int>::iterator rex = (it->ESRules).begin(); rex != (it->ESRules).end(); rex++) {
//                G_NLP[*rex].output(stdout);
//            }
//            printf("\nin_U(P) : \n");
//            for(set<int>::iterator iit = inUP.begin(); iit != inUP.end(); iit++) {
//                G_NLP[*iit].output(stdout);
//            }
...
//            printf("\nLoop : \n");
//            HLUPX.at(i-1).print(stdout);
//            printf("\nR^-(E,X):\n");
//            for(set<int>::iterator iit = ((HLUPX.at(i-1)).ESRules).begin(); iit != ((HLUPX.at(i-1)).ESRules).end(); iit++) {
//                G_NLP[*iit].output(stdout);
//            }
...
set< set<int> > DependenceGraph::callClaspD(string fileName) {
    ...
    string cmd = "gringo " + fileName + " | clasp 1 > " + OUTPUT_FILE;
    char buff[BUFF_SIZE];
...
//            fprintf(stdout, "%s ", Vocabulary::instance().getAtom(*sit));
//        }
//        fprintf(stdout, "\n");
//    }
//    fprintf(stdout, "\n");  fflush(out);
...
void DependenceGraph::application(FILE* out) {
    ...
    trpPU(ctup);
    fprintf(out, "\nct_U(P) after tr_p : \n");
    for(vector<Rule>::iterator it = ctup.begin(); it != ctup.end(); it++)
        (*it).output(out);
}