...
void createPealGraph(int PEALNUM, int PEALSIZE) {
    ...
    do {
        ed = rand() % PEALSIZE + (PEALNUM-1)*PEALSIZE;
    }while(visited[ed]);
    visited[ed] = true;     cuts.insert(ed);
    fprintf(fout, "arc(%d, %d).\n", ed, st);    fflush(fout);

    // other first node to its next node
    for(int i = 0; i < PEALNUM-1; i++) {
        do {
            st = rand() % PEALSIZE + i*PEALSIZE;
        }while(visited[st]);
        visited[st] = true; cuts.insert(st);

        do{
            ed = rand() % PEALSIZE + (i+1)*PEALSIZE;
        }while(visited[ed]);
        visited[ed] = true;     cuts.insert(ed);
        fprintf(fout, "arc(%d, %d).\n", st, ed);    fflush(fout);
    }
...
int main(int argc, char** argv) {
    ...
    createPealGraph(4, 8);
    createPealGraph(4, 9);
    createPealGraph(4, 10);
    
    createPealGraph(5, 6);
    createPealGraph(5, 7);
    createPealGraph(5, 8);
    createPealGraph(5, 9);
    createPealGraph(5, 10);
    createPealGraph(5, 11);
